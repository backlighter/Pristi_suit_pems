...
class Survey_Dataset(Dataset):
    def __init__(self,true_data,ob_mask,gt_mask, c_data,val_start, test_start, eval_length=12, mode="train", missing_pattern='block',
        ...
        if mode == 'train':
            ...
            self.observed_mask = np.ones_like(ob_mask[test_start:]) # 构造 只含有 原本缺失
            self.gt_mask = ob_mask[test_start:]  #在observed_mask基础上 每个原本存在点构造%50缺失

            # self.observed_mask 真实数据
            # self.gt_mask：每个节点人为mask的部分
            #构造ob_mask  block 缺失 point 缺失 %50
...
def get_dataloader(batch_size, device, val_len=0.2, test_len=0.2, missing_pattern='block',
    ...
    #eval_mask代表 Pristi构造的缺失%5左右 1-ob_mask代表 之前人工构造的缺失(1代表缺失，0代表有值)
    #((eval_mask | (1-ob_mask))代表所有的缺失、1-(eval_mask | (1-ob_mask))代表有值的地方
