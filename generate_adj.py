...
def get_similarity(distance_df_filename,node_num=307,thr=0.1):
    adj = np.zeros((node_num,node_num))  # 432*432
...
def get_similarity_metrla(thr=0.1, force_symmetric=False, sparse=False):
    dist = np.load('../../data/METR-LA/adj.npy')
...
def get_similarity_pemsbay(thr=0.1, force_symmetric=False, sparse=False):
    dist = np.load('../../data/PEMS_BAY/adj.npy')
    finite_dist = dist.reshape(-1)
    finite_dist = finite_dist[~np.isinf(finite_dist)]
    sigma = finite_dist.std()
    adj = np.exp(-np.square(dist / sigma))
...
def compute_support_gwn(adj, device=None): #计算邻接矩阵的转置 返回包含两个张量的列表，分别是原始邻接矩阵和转置后的邻接矩阵
