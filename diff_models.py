...
class Guide_diff(nn.Module):
    def __init__(self, config, inputdim=1, target_dim=36, is_itp=False): #is_itp=use_guide
        super().__init__()
        ...
        if self.is_itp:
            ...
            self.itp_projection = Conv1d_with_init(inputdim-1, self.itp_channels, 1) #一个一维卷积
            #搞不懂你输出的是个啥
        ...
        self.support = compute_support_gwn(self.adj, device=config["device"]) ##计算邻接矩阵的转置 返回包含两个张量的列表，分别是原始邻接矩阵和转置后的邻接矩阵
    ...
    def forward(self, x, side_info, diffusion_step, itp_x, cond_mask):
        ...
        x = F.relu(x)
        x = x.reshape(B, self.channels, K, L)  # H^in
