...
class DiffusionEmbedding(nn.Module):
    def __init__(self, num_steps, embedding_dim=128, projection_dim=None):
        ...
        self.register_buffer( #名称（"embedding"）：这是注册缓冲区的名字，可以通过 self.embedding 来访问这个缓冲区。
            "embedding",
            self._build_embedding(num_steps, embedding_dim / 2),
            persistent=False,
        )
        self.projection1 = nn.Linear(embedding_dim, projection_dim) #投影层1
        self.projection2 = nn.Linear(projection_dim, projection_dim) #投影层2

    def forward(self, diffusion_step):
        x = self.embedding[diffusion_step] #从 "embedding" 缓冲区中选择与当前扩散步骤对应的嵌入向量。
        x = self.projection1(x)
        x = F.silu(x) #SiLU（Sigmoid Linear Unit，也称为 Swish）激活函数处理线性层的输出。
    ...
    def _build_embedding(self, num_steps, dim=64):
        ...
        #生成一个从0到 num_steps-1 的整数序列，并将其形状改为 (num_steps, 1)
        #计算一个频率序列，用于调整步骤序列中每个元素的尺度。这个频率基于对数尺度，范围从 10^0 到 10^4
...
class TemporalLearning(nn.Module):
    def __init__(self, channels, nheads, is_cross=True):
        ...
        self.time_layer = Attn_tem(heads=nheads, layers=1, channels=channels)  #Transformer
...
class GuidanceConstruct(nn.Module):
    def __init__(self, channels, nheads, target_dim, order, include_self, device, is_adp, adj_file, proj_t):
        super().__init__()
        self.GCN = AdaptiveGCN(channels, order=order, include_self=include_self, device=device, is_adp=is_adp, adj_file=adj_file)
        self.attn_s = Attn_spa(dim=channels, seq_len=target_dim, k=proj_t, heads=nheads) #空间注意力
        self.attn_t = Attn_tem(heads=nheads, layers=1, channels=channels) #时间注意力
        self.norm1_local = nn.GroupNorm(4, channels)
